...
export const loadAssignees = async (appConfig: AppContextType) => {
  ...
  try {
    ...
    const clientRes = await fetch(
      `https://api-beta.copilot.com/v1/client`,
      copilotGetReq,
    );

    const companyRes = await fetch(
      `https://api-beta.copilot.com/v1/company`,
      copilotGetReq
    )
...
const handlePostConfig = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const userConfigUploadParams = {
    ...bucketParams, Key: `users/${userEmail}/apps.json`, Body: JSON.stringify([
      configId,
    ])
  };
  try {
